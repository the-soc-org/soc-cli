...
# Sources user_config.sh, source_config.sh, and system_config.sh from the
# current working directory. Aborts with an error message if any file is
# missing. Usage: source_config_files
source_config_files() {
  if [[ -f 'user_config.sh' && -f 'source_config.sh' && -f 'system_config.sh' ]]; then
    ...
    echo "file is missing. Please run the 'soc init' command to create the "
    echo "missing file(s)."
    exit 1
  fi
}
